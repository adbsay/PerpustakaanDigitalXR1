...
// POST /api/publisher/auth/forgot-password
export async function POST(req: NextRequest) {
  // Rate limit: 5 requests per 15 minutes per IP
  const ip = getClientIp(req);
  const rateLimitResult = checkRateLimit(`forgot-pwd:${ip}`, { windowMs: 15 * 60 * 1000, max: 5 });
  ...
  try {
    ...
    const resetUrl = `${req.nextUrl.origin}/publisher/auth/forgot-password?token=${resetToken}`;

    return successResponse({
      message: 'Instruksi reset password telah disiapkan.',
      resetUrl, // Provided for instant direct testing in development
      token: resetToken,
    });
  } catch (error) {
